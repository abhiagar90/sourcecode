...
import simpledb.index.HeightManageBtree;
...
public class BTreeIndex implements Index {
	...
	public BTreeIndex(String idxname, Schema leafsch, Transaction tx) {
		...
		if (page.getNumRecs() == 0) {
			...
			Constant minval = (fldtype == INTEGER) ? new IntConstant(
					Integer.MIN_VALUE) : new StringConstant("");
	...
	public void insert(Constant dataval, RID datarid) {
		...
		if (e2 != null) {
			root.makeNewRoot(e2);
			int oldheight = new HeightManageBtree().getHeight(indexname);
			new HeightManageBtree().updateHeight(oldheight+1, this.indexname);
			// after successfully made new root
		}
		root.close();
	}
	...
	public int getHeight() {
		return new HeightManageBtree().getHeight(indexname);
	}
}
