...
public class IndexUpdatePlanner implements UpdatePlanner {
	...
	public int executeInsert(InsertData data, Transaction tx) {
		...
		Map<String, IndexInfo> indexes = SimpleDB.mdMgr().getIndexInfo(tblname,
				tx);
		...
		for (String fldname : data.fields()) {
			...
			// TODO: remove code
			System.out.println("checking for indexes on this field " + fldname);

			IndexInfo ii = indexes.get(fldname);

			// TODO: remove code
			System.out.println("index for this field is null? " + ii);

			//Task 1: When inserting a new entry check the type and call appropriate index in Index: Done
			if (ii != null) {
				Index idx = ii.open();
				System.out.println("actual index object while inserting: " +idx);
				idx.insert(val, rid);
				idx.close();
			}
		}
	...
	public int executeModify(ModifyData data, Transaction tx) {
		...
		while (s.next()) {
			...
			//TODO: remove
			System.out.println("Modifying a record with an index? "+idx);
			if (idx != null) {
				RID rid = s.getRid();
